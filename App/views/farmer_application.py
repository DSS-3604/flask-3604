...
from App.controllers.user import is_admin, is_farmer, get_user_by_email, get_user_by_username
...
def create_farmer_application_action():
    ...
    if is_admin(current_identity) or is_farmer(current_identity):
        return jsonify({"message": "You are not authorized to create a farmer application"}), 403
    if not data['comment']:
